...
_URL = "https://raw.githubusercontent.com/youssefEmadabd/traindata/master/train_data_clean.csv"
...
class ArabicEmpConv(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, filepath):
        with open("train_data_clean.csv",'r',encoding='utf-8') as f:
            csv_reader = csv.reader(f)
            for i , row in enumerate(csv_reader):
                if i==0:
                    continue
                yield i-1, {
                    "emotion": arabert_prep.preprocess(row[1]),
                    "context": arabert_prep.preprocess(row[2]),
                    "response": row[3]
                }